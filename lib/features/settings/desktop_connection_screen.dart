...
class _DesktopConnectionScreenState extends State<DesktopConnectionScreen> {
  ...
  Future<void> _pair() async {
    ...
    _message = 'Paired. Enable Desktop analysis below only when you want to send text to this computer.';
  }
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    body: ListView(padding: const EdgeInsets.all(20), children: [
      const Text('Connect only to a computer you trust. Obtain pairing data directly from PrivacyGate Desktop → Mobile Devices. Never paste pairing data from an unknown sender.'),
      ...
      if (_credential != null) ...[
        ...
        SwitchListTile(
          ...
          subtitle: const Text('Sends original text over encrypted TLS to this PC when scanning. Restore mappings stay on this device. Disabled again after restarting the app.'),
          value: widget.client.analysisEnabled,
          onChanged: _busy ? null : (value) => setState(() => widget.client.analysisEnabled = value),
        ),
        OutlinedButton(onPressed: _busy ? null : () => _run(() async {
          final online = await widget.client.checkConnection();
          _message = online ? 'Desktop reachable and pairing accepted.' : 'Desktop did not accept this pairing.';
        }), child: const Text('Check connection')),
        TextButton(onPressed: _busy ? null : () => _run(() async {
          widget.client.analysisEnabled = false;
          await widget.client.credentials.delete();
          _credential = null;
          _message = 'Forgot connection locally. Revoke this device on Desktop as well.';
        ...
        const SizedBox(height: 12),
        TextField(controller: _bundle, minLines: 4, maxLines: 8,
          autocorrect: false, enableSuggestions: false,
          decoration: const InputDecoration(labelText: 'Pairing data (JSON)', helperText: 'Temporary pairing data contains a secret. Do not share publicly.')),
        const SizedBox(height: 12),
        FilledButton(onPressed: _busy ? null : () => _run(_pair), child: const Text('Pair with Desktop')),
      ],
      const SizedBox(height: 20),
      const Text('Library transfer is not enabled in this version. Pairing does not grant access to the Desktop Library.'),
    ]),
  );
}
